...
@click.option('-d', '--delay', type=int, default=0,
              help=('Time (s) between debugging tags with the same EPC (default 0)'))
...
def inventory(host, port, time, report_every_n_tags, antennas, tx_power,
              ...
              tag_population, reconnect, delay, tag_filter_mask,
    ...
    Args = namedtuple('Args', ['host', 'port', 'time', 'every_n', 'antennas',
                               ...
                               'reconnect', 'delay', 'tag_filter_mask',
                               'impinj_extended_configuration',
                               'impinj_search_mode',
                               'impinj_reports',
                               'impinj_fixed_freq'])
    args = Args(host=host, port=port, time=time, every_n=report_every_n_tags,
                ...
                reconnect=reconnect, delay=delay, tag_filter_mask=tag_filter_mask,
